(function ($) {
  ...
  (function loadMoreProducts() {
    var page = 2;
    $("#loadMoreProductsBtn").on("click", function (e) {
      e.preventDefault();

      $("#loadMoreProductsBtn").hide();
      $("#loadingProductSpinner").show();

      // // Simulate AJAX load delay
      // setTimeout(function () {
      //   $("#loadingProductSpinner").hide();
      //   $("#loadMoreProductsBtn").show();

      //   // TODO: append new content here
      // }, 2000);
      ...
      fetch("load_more_products?page=" + page, {
        ...
        .then((res) => res.json())
        .then((data) => {
          document.querySelector("#product-container").innerHTML += data.html;
          $("#loadingProductSpinner").hide();
          if (data.next === null) {
            $("#loadMoreProductsBtn").hide();
          } else {
